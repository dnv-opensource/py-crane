#!/usr/bin/env python3
"""
Test script for real-time interactive crane commands with independent verification
"""
...
def test_continuous_commands():
    ...
    for i, cmd in enumerate(commands, 1):
        ...
        print(f"[INFO] Waiting for command execution...")
        ...
        for wait_step in range(25):
            ...
            print(f"  [{elapsed:.1f}s] Timeline frames: {status['timeline_frames']}, Active commands: {status['active_commands']}")
            
            if status['motion_detected']:
                motion = status['motion_detected']
                print(f"  Motion: Boom_len_delta={motion['boom_length_delta']:+.3f}m, Polar_delta={motion['polar_delta']:+.4f}rad, Az_delta={motion['azimuth_delta']:+.4f}rad, Wire_delta={motion['wire_delta']:+.3f}m")
                last_motion = motion
            ...
            if status['active_commands'] == 0 and status['timeline_frames'] > 50:
                if last_motion:
                    ...
                    print(f"  Final position: len={status['current_position']['boom_length']:.2f}m, polar={status['current_position']['polar_angle']:.3f}rad, azimuth={status['current_position']['azimuth']:.3f}rad")
                break
    ...
    for retry in range(10):
        try:
            ...
            if response.status_code == 200:
                ...
                video_path = f"/nvme/admhonqia/New/py-crane/results/test_output.mp4"
                with open(video_path, 'wb') as f:
                    f.write(response.content)
                print(f"[SUCCESS] Video saved to: {video_path}")
                break
            else:
                status_info = response.json() if response.headers.get('content-type') == 'application/json' else response.text
...
if __name__ == "__main__":
    print("""
================================================================================
CRANE SERVER REAL-TIME INTERACTIVE TEST
================================================================================

This test will:
  1. Send 4 commands sequentially (each with 5 seconds duration)
  2. Verify execution through debug API (without waiting for video)
  3. Display state changes to confirm commands are working
  4. Wait for video generation and download if successful
  
The verification is independent of video output - you will see command
execution confirmation through position changes even if video generation fails.
================================================================================
    """)
    ...
    try:
        ...
        print(f"[INFO] Initial state - Boom: {status['length']:.2f}m, Polar: {status['polar']:.3f}rad, Azimuth: {status['azimuth']:.3f}rad")
    except Exception as e:
