...
def make_mobile_crane_fmu(only_path: bool = True):
    if not only_path:
        ...
        fmu_path = Path(__file__).parent / "resources" / "MobileCrane.fmu"
...
def test_run_mobilecrane_move(mobile_crane_fmu, show: bool):
    result = simulate_fmu(
        ...
        start_values={
            ...
            "boom_boom[0]": 8,
            "boom_boom[1]": 45.0,  # input as deg, internal: rad
            "pedestal_angularVelocity[1]": 1.0,  # azimuthal movement 1 deg per time step
            "rope_boom[0]": 1e-6,
    ...
    for i, row in enumerate(result):
        ...
        assert abs(row[19] - 8 / sqrt(2) * cos(radians(row[0]))) < 1e-9
